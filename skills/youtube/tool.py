...
def _get_youtube_service():
    """Google OAuth2 인증으로 YouTube Data API v3 서비스를 반환합니다.

    Gmail/Calendar와 동일한 credentials.json/token.json을 사용합니다.
    Google Cloud Console에서 YouTube Data API v3가 활성화되어 있어야 합니다.
    """
...
def youtube_video_info(video_id: str) -> str:
    ...
    try:
        ...
        return (
            ...
            f"\n📊 통계\n"
            f"조회수: {_format_number(stats.get('viewCount', 0))} | "
            f"좋아요: {_format_number(stats.get('likeCount', 0))} | "
            f"댓글: {_format_number(stats.get('commentCount', 0))}\n"
            f"\n🏷️ 태그: {tags_str or '없음'}\n"
            f"\n📝 설명:\n{desc_preview}\n"
...
def youtube_channel_info(channel_query: str) -> str:
    ...
    try:
        ...
        subscriber_str = f"{_format_number(subscriber_count)}명" if subscriber_count else "비공개"
        ...
        return (
            ...
            f"\n📊 통계\n"
            f"구독자: {subscriber_str} | 동영상: {_format_number(stats.get('videoCount', 0))}개 | "
            f"총 조회수: {_format_number(stats.get('viewCount', 0))}\n"
...
def youtube_trending(
    ...
    Args:
        ...
        category_id: 카테고리 ID (0=전체, 10=음악, 15=동물, 17=스포츠, 20=게임, 22=블로그, 23=코미디, 24=엔터테인먼트, 25=뉴스, 28=과학기술)
        max_results: 반환할 결과 수 (1~50, 기본값: 10)
    """
    try:
        youtube = _get_youtube_service()
        ...
        _CATEGORY_NAMES = {
            ...
            22: "블로그/Vlog", 23: "코미디", 24: "엔터테인먼트", 25: "뉴스/정치", 28: "과학/기술",
        }
        region_flag = {
            "KR": "🇰🇷", "US": "🇺🇸", "JP": "🇯🇵", "GB": "🇬🇧",
            "DE": "🇩🇪", "FR": "🇫🇷", "IN": "🇮🇳", "BR": "🇧🇷",
...
def youtube_comments(
    ...
    try:
        ...
        resp = youtube.commentThreads().list(
            ...
            maxResults=max_results, order=order, textFormat="plainText",
        ).execute()
        items = resp.get("items", [])
        ...
        for i, item in enumerate(items, 1):
            ...
            lines.append(f"{i}. 👤 {author} ({published}) | 👍 {likes}{reply_str}\n   {text}")
        return "\n\n".join(lines)
    except Exception as e:
...
def youtube_channel_videos(
    ...
    try:
        ...
        order_label = {"date": "최신순", "viewCount": "조회수순"}.get(order, order)
        lines = [f"📺 채널 동영상 목록\n채널 ID: {channel_id} | URL: {channel_url}\n정렬: {order_label} | {len(items)}개 표시\n"]
